...
// Secours si DONNEES_DETECTIVE_BOOSTE n'est pas explicitement dans data.js
const DONNEES_DETECTIVE_BOOSTE = data.DONNEES_DETECTIVE_BOOSTE || {
  suspects: ["Andy", "Titan", "Mox", "Alex", "Sarah"],
  lieux: ["Cuisine", "Salon", "Jardin", "Garage", "Chambre"],
  armes: ["Couteau", "Pistolet", "Corde", "Poison", "Marteau"]
};
      ...
      if (statusCode === DisconnectReason.loggedOut) {
        ...
        if (fs.existsSync('./auth_info')) {
          try {
            ...
            console.error("Erreur lors de la suppression de auth_info:", e);
          }
        }
      }
...
💖 *.love* [@mention] ➔ Test de compatibilité
...
🛑 *.stop* ➔ Réinitialiser le jeu actif`;
      ...
      if (lowerText.startsWith('.pp')) {
        ...
        try {
          ...
          await envoyerAvecDelai(sock, remoteJid, { image: { url: ppUrl }, caption: `📸 Photo de profil de *@${cible.split('@')[0]}*`, mentions: [cible] }, { quoted: msg }, msg);
        } catch (e) {
      ...
      if (lowerText.startsWith('.love')) {
        ...
        if (mention) {
          ...
          txt += `👤 Compatibilité de *@${senderJid.split('@')[0]}*\n`;
        }
        txt += `📊 Score : ${genererBarreHP(score, 100)} (${score}%)\n💬 *Avis :* ${comm}`;

        await envoyerAvecDelai(sock, remoteJid, { text: txt, mentions: mention ? [senderJid, mention] : [senderJid] }, { quoted: msg }, msg);
        return;
      }
...
// ==========================================
// 🛠️ DÉCLENCHEURS ET FONCTIONS DES JEUX
// ==========================================
