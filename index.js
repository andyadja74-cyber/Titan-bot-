...
// ==========================================
// 1. SERVEUR WEB & KEEP-ALIVE 24/7 (RENDER & UPTIMEROBOT)
// ==========================================
app.get("/", (req, res) => {
  res.send("🤖 BOT TITAN ULTIMATE EST ACTIF ET EN LIGNE 24/7 !");
});

// Route /health requise par UptimeRobot
...
// ==========================================
// 📂 LIAISON DES BANQUES DE DONNÉES GITHUB (base.js & bac.js)
// ==========================================
let base = {};
let bac = {};
...
try {
  base = require('./base');
  console.log("🟢 Banque de données 'base.js' chargée avec succès !");
} catch (err) {
  console.warn("⚠️ 'base.js' non trouvé ou erreur au chargement :", err.message);
}

try {
  bac = require('./bac');
  console.log("🟢 Banque de données 'bac.js' chargée avec succès !");
} catch (err) {
  console.warn("⚠️ 'bac.js' non trouvé ou erreur au chargement :", err.message);
}
...
// Dictionnaires de secours (si non présents dans base.js)
const EMOJIS_DICO = base.EMOJIS_DICO || ["🦁", "🍕", "🚀", "👑", "⚽", "🎮", "🎸", "🥑", "💎", "🔥", "🦄", "🎯", "🤖", "👻", "🌮", "🐉", "🍔", "🏆", "🎨", "🎰"];
const CAPITALES_DICO = base.CAPITALES_DICO || [
...
const CITATIONS_MANGA = base.CITATIONS_MANGA || [
    ...
    try {
      ...
      if (partieActive && partieActive.statut === 'EN_COURS') {
        ...
        if (estUnReponseJeu) {
          ...
          // 🔤 JEU BACCALAURÉAT (Utilise bac.js)
          if (partieActive.type === 'BACCALAURÉAT') {
            if (typeof bac.verifierReponse === 'function') {
              const resBac = bac.verifierReponse(partieActive, tentativeReponse, senderJid);
              if (resBac) {
                await sock.sendMessage(remoteJid, { text: resBac }, { quoted: msg });
                return;
              }
            }
          }
...
👉 **.menu**`;
      ...
      if (lowerText === '.jouer') {
        ...
        // Lancement BACCALAURÉAT via bac.js
        if (party.type === 'BACCALAURÉAT' && typeof bac.demarrerMancheBac === 'function') {
          party.statut = 'EN_COURS';
          party.manche += 1;
          demarrerTimerInactivite(sock, remoteJid);

          const msgBac = bac.demarrerMancheBac(party);
          await sock.sendMessage(remoteJid, { text: msgBac, mentions: party.joueurs.map(j => j.jid) });
          return;
        }
...
💡 *ASTUCE JEU :* En partie, mettez toujours **@** devant votre réponse (ex: *@paris*) pour que le bot la prenne en compte ! Vous pouvez parler normalement sans gêner le jeu.`;
      ...
      // ----------------------------------------------------
      // 🕹️ COMMANDES DÉLÉGUÉES A BASE.JS / BAC.JS OU NATIVES
      // ----------------------------------------------------

      // Exécution prioritaire via base.js si une fonction générique de commande existe
      if (typeof base.executerCommande === 'function') {
        const commandeTraitee = await base.executerCommande(sock, remoteJid, senderJid, cleanText, msg, { partiesEnCours, isGroup, xpDatabase });
