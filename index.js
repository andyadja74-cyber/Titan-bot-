...
// ⚙️ CONFIGURATION DU BINAIRE FFMPEG EMBARQUÉ
...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      // 🎙️ CONVERSION TEXTE EN VOCAL OPTIMISÉE WHATSAPP (OGG/OPUS REAL PTT)
      if (lowerText.startsWith('.voc')) {
        ...
        try {
          ...
          await envoyerAvecDelai(sock, remoteJid, { 
            text: `❌ Impossible de générer la note vocale : ${err.message}` 
          }, { quoted: msg }, msg);
        }
        return;
      }
      ...
      if (lowerText.startsWith('.gemini') || lowerText.startsWith('.ai')) {
        ...
        if (!GEMINI_API_KEY || GEMINI_API_KEY === "ENTREZ_VOTRE_CLE_API_GEMINI_ICI") {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ *CLÉ API MANQUANTE :* Mettez à jour la variable GEMINI_API_KEY dans votre fichier `index.js` ou sur Render." }, { quoted: msg }, msg);
          return;
        }
        ...
        try {
          ...
          await envoyerAvecDelai(sock, remoteJid, { text: `❌ Erreur de communication avec Gemini API (${err.message || 'Clé non valide'}).` }, { quoted: msg }, msg);
        }
        return;
      }
