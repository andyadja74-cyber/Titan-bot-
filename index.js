    ...
    try {
      ...
      if (lowerText.startsWith('.voc')) {
        ...
        try {
          ...
          // Conversion du buffer MP3 en OGG/Opus compatible avec tous les appareils WhatsApp
          ...
          await envoyerAvecDelai(sock, remoteJid, { 
            text: "❌ Impossible de générer la note vocale. Assurez-vous que FFmpeg est installé sur votre serveur." 
          }, { quoted: msg }, msg);
        }
        return;
      }
      ...
      if (lowerText.startsWith('.gemini') || lowerText.startsWith('.ai')) {
        ...
        if (!GEMINI_API_KEY || GEMINI_API_KEY === "ENTREZ_VOTRE_CLE_API_GEMINI_ICI") {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ *CLÉ API MANQUANTE :* Veuillez remplacer `ENTREZ_VOTRE_CLE_API_GEMINI_ICI` par votre clé API dans `index.js`." }, { quoted: msg }, msg);
          return;
        }
        ...
        try {
          ...
          await envoyerAvecDelai(sock, remoteJid, { text: "❌ Erreur de communication avec Gemini API." }, { quoted: msg }, msg);
        }
        return;
      }
...
⚡ *━━━━━━━━━━━━━━━━━━━━━━━━━* ⚡`;
      ...
      if (lowerText === '.notes') {
        ...
        await envoyerAvecDelai(sock, remoteJid, { text: "🔒 Entrez le mot de passe @Ashley pour afficher vos notes :" }, { quoted: msg }, msg);
        return;
      }
